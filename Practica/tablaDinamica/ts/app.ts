...
$(document).ready(function () {
    ...
    $.get("http://localhost:3000/personas",function (data,status) {
        ...
        let seccionPersonas:string = "";


        for (let i = 0; i < personasCompleto.length; i++) {
            ...
            $("input").change(function () {
                ...
                let idSeleccionado:string = $(this).parent().siblings(":first").text();
        ...
        $("#tBodyTable").dblclick(function (e) {
           ...
           let idTarget = $(e.target).closest("td").prev().text();
           let nombreTarget = $(e.target).closest("td").text();
           let apellidoTarget = $(e.target).closest("td").next().text();
           let fechaTarget = $(e.target).closest("td").next().next().text();

           let sexoTarget = $(e.target).closest("td").next().next().next().text();
