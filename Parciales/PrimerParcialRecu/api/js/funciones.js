...
var usuarioGlobal = { "id":"","nombre":"","apellido":"","fecha":"","estado":"","avatar":"" }
...
$(document).ready(function () {
    ...
    $.get("http://localhost:3000/personajes",function (data, status) {
        ...
        var personasCompleto = data;
            ...
            var seccionPersonas = "";
            var estadoContrarioPersonaje;
           
            

            for(var i=0; i< personasCompleto.length; i++)
            {
                if(personasCompleto[i].estado == "Vivo")
                {
                    estadoContrarioPersonaje = "Muerto";
                }
                else{
                    estadoContrarioPersonaje = "Vivo";
                }
                ...
                seccionPersonas += "<tr>  <td hidden>"+ personasCompleto[i].id       + "</td>" +
                                          ...
                                          "<td>" + 
                                          "<select class='selectEstado'>" +
                                          "<option value='"+personasCompleto[i].estado+"' selected>"+personasCompleto[i].estado+"</option>"+
                                          "<option id='estadoContrario' value='"+estadoContrarioPersonaje+"'>"+estadoContrarioPersonaje+"</option>"+                                          
                                          "</select>"
                ...
                $("select.selectEstado").change(function () { 
                    // var optionPost=estadoContrarioPersonaje;
                    var targetInput = event.target;
                    var idSeleccionado = $(this).parent().siblings(":first").text();
...
function transicionSpinner(vienedeEstado) {
